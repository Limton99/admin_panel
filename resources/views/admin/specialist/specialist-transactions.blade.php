        ...
        <div class="content-body">
            <div id="user-profile">
                <section id="timeline" class="timeline-center timeline-wrapper">
                    <h1>Баланс на щете: {{$specialist['specialist']->wallets->balance}}</h1>
                    <div class="card" style="padding: 20px; margin-left: 10px">
                        <table class="table table-striped table-bordered selection-multiple-rows dataTable" id="DataTables_Table_4" role="grid" aria-describedby="DataTables_Table_4_info">
                            ...
                            <tbody>
                            @foreach($specialist['transactions'] as $data)
                                <tr role="row" class="odd">
                                    <td class="sorting_1"><a href="{{route('order', $data->id)}}">{{$data->order->order_number}}</a></td>
                                    @foreach($data->order->transaction as $transaction)
{{--                                        {{dd($transaction)}}--}}
                                        <td>
                                            {{$transaction->status}}
                                        </td>
                        ...
                        {{$specialist['transactions']->links("pagination::bootstrap-4")}}
                    </div>
                </section>
            </div>
